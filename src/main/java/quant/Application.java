...
public class Application {
    ...
    public Future<String> calculateAsync(String symbol) throws InterruptedException {
...
Map<String, Map<String, String>> signals  = initData.get(symbol);
            ...
            completableFuture.complete(symbol+"done.");
            return null;
        });
...
//	MathOperatorService2 mathService = getMathOperatorService;
    ...
    public Future<String> calculateAsyncSymbol(String symbol,DateFormat df,InputParameterSet params,DateTimeFormatter fmt,RuleImplService util,MathOperatorService2 mathService) throws InterruptedException {
        CompletableFuture<String> completableFuture 
          = new CompletableFuture<>();
...
//
        	...
        	//******************************************************************************************************************
    		//async ends here.
            completableFuture.complete("load and compute "+symbol+"  done.");
            return null;
        });
    ...
    //the key is the date
    public void transform() throws InterruptedException, ExecutionException
    {
    	for(String symbol: initData.keySet())
    	{
    		Future<String> completableFuture = calculateAsync(symbol);
...
//			long startTime_ = System.nanoTime();
			for (String symbol : symbols) {
				...
				Future<String> completableFuture = calculateAsyncSymbol(symbol, df, params, fmt, util, mathService);
				 
	    		String result = completableFuture.get();
	    		System.out.println(result);
				
				
				//sync code starts here
				//*********************************************************************************************************************
